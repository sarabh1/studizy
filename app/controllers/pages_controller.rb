class PagesController < ApplicationController
  ...
  def dashboard
    ...
    @yellow_sessions = @sessions_fourth.select {|session| session.start_date > DateTime.now + 3 && session.start_date < DateTime.now + 6}
    @green_sessions = @sessions_fourth.select {|session| session.start_date > DateTime.now + 6}
  end
